...
CFL <- read.csv("~/Dropbox/PhD Program/Dissertation_Research/2_Fuels/Fuel_Output_Sheets/SFuels_R/CFL.csv")
CC <- read.csv("~/Dropbox/PhD Program/Dissertation_Research/2_Fuels/Fuel_Output_Sheets/SFuels_R/CC.csv")
source('~/Dropbox/PhD Program/Dissertation_Research/2_Fuels/Fuel_Output_Sheets/SFuels_R/NestAnovaPerm.txt')
#attach(CFL)
...
CFL.2 <- CFL
CFL <-subset(CFL, S.F=="S")
CFL <-subset(CFL, Species=="PIEN")
#--------------------------------------------------
#Aggregate all plots to site level
CFL.cpy <- aggregate(CBH ~ Site + Year + Age + Plot, data=CFL.2, FUN=mean)
...
CC.site <- aggregate(CanopyOpen ~ Year +Age, data=CC, FUN=min)

CFL.plot <- aggregate(cbind(LiFol,DeFol,Li.1hr,De.1hr,Li.10hr,De.10hr,Li.100hr,De.100hr) ~ Site + Year + Age +Plot, data=CFL, FUN=sum)

CFL.site <- aggregate(cbind(LiFol,DeFol,Li.1hr,De.1hr,Li.10hr,De.10hr,Li.100hr,De.100hr) ~ Site + Year + Age, data=CFL.plot, FUN=mean)
...
CFL.basum$BA_g50pct.nd <- CFL.basum$BA_Gr + CFL.basum$BA_Ye + CFL.basum$BA_Nd
CFL.bamean$BA_g50pct.nd <- CFL.bamean$BA_Gr + CFL.bamean$BA_Ye + CFL.bamean$BA_Nd
CFL.basum$BA_l50pct.tw <- CFL.basum$BA_Br + CFL.basum$BA_Sn

#Attach the new calculations to the dataframe
CFL.sum$BA_g50pct.nd <- CFL.basum$BA_g50pct.nd
CFL.sum$BA_l50pct.tw <- CFL.basum$BA_l50pct.tw
CFL.mean$BA_g50pct.nd <- CFL.bamean$BA_g50pct.nd
...
for(i in c("LiFol","DeFol","Li.1hr","De.1hr","Li.10hr","De.10hr","Li.100hr","De.100hr"))
  TukeyHSD1[[i]] <- TukeyHSD(aov1[[i]], "SB.Age", ordered=T)
for(i in c("LiFol","DeFol","Li.1hr","De.1hr","Li.10hr","De.10hr","Li.100hr","De.100hr")) 
...
for(i in c( "EndemicOld", "EndemicYoung","1940Old", "1960Old", "1990Old", "2000Old", "2010Old", "1990Young", "2000Young")){
  ...
  CV1[[i]] <- cv.model(t)
  SE1[[i]] <- sd(q$AFCL)/sqrt(length(q$AFCL))
}
...
mtext("(a) F(9,32) = 24.17, p = < 0.0001", 1, adj=0.1, las=1, padj = -18,cex = 0.75)
...
bg.DeFol = with(CFL.plot, bargraph.CI(x.factor=Year, group=Age, response=DeFol,family = "Times",
             lc=FALSE, err.width = .03,  ylab = "Dead Foliage (MG/ha)",ylim=c(0,0.5),
...
mtext("(b) F(9,33) = 4.14, p = 0.0014", 1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(d) F(9,33) = 28.25, p = < 0.0001",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(e) F(9,33) = 1.54, p = 0.1780", 1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(f) F(9,33) = 7.39, p = < 0.0001",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(g) F(9,33) = 32.82, p = < 0.0001",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])

bg.De.10hr = with(CFL.plot, bargraph.CI(x.factor=Year, group=Age, response=De.10hr,family = "Times",
             lc=FALSE, err.width = .03, ylab = "Dead 10hr (MG/ha)",ylim=c(0,4),
             legend=F, x.leg=3.3, cex.leg=1.2, cex.names=1, cex.lab = 1.2,axisnames=F, col= c("dark gray", "white")))
...
mtext("(h) F(9,33) = 7.28, p = < 0.0001",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(j) F(9,33) = 40.39, p = < 0.0001",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])

bg.De.100hr = with(CFL.plot, bargraph.CI(x.factor=Year, group=Age, response=De.100hr,family = "Times",
             lc=FALSE, err.width = .03, xlab="Year of SB Outbreak", ylab = "Dead 100hr (MG/ha)",ylim=c(0,1),
...
mtext("(k) F(9,33) = 3.76, p = 0.0027", 1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
...
mtext("(l) F(9,33) = 2.53, p = 0.0450",  1, adj=0.1, las=1, padj = -18,cex = 0.75)
box(lty = 'solid', col = 'black')
Axis(side=1, labels=F, at=bg.LiFol$xvals[c(1,3,5:12)])
par(op)
dev.off()
...
op <- par(family = "Times")
  ...
  mtext("F(9,33) = 7.78, p = < 0.0001", 2, adj=-0.1, las=1, padj = -7,cex = 0.55)
  box(lty = 'solid', col = 'black')
  ...
  mtext("F(9,33) = 6.45, p = < 0.0001", 2, adj=-0.1, las=1, padj = -7,cex = 0.55)
  box(lty = 'solid', col = 'black')
  Axis(side=1, labels=F, at=bg.li$xvals[c(1,3,5:12)])
  ...
  mtext("F(9,33) = 5.28, p = 0.0002", 2, adj=-0.1, las=1, padj = -7,cex = 0.55)
  box(lty = 'solid', col = 'black')
  Axis(side=1, labels=F, at=bg.li$xvals[c(1,3,5:12)])
  ...
  mtext("F(9,33) = 8.20, p = < 0.0001", 2, adj=-0.1, las=1, padj = -7,cex = 0.55)
  box(lty = 'solid', col = 'black')
  Axis(side=1, labels=F, at=bg.li$xvals[c(1,3,5:12)])  
par(op)
dev.off()
